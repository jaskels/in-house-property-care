  ...
  const estimate = useMemo(() => {
    ...
    if (form.quoteType === 'mowing') {
      ...
      if (form.mowStyle === 'bagHaulAway') {
        perVisit = (form.frequency === 'weekly' ? selectedRange.weeklyBagLeave : selectedRange.biweeklyMulch) + bagHaulAwayUpcharge
      }

      if (form.dogWaste) perVisit += dogWastePerVisit
      if (form.overgrownFirstCut) oneTime += overgrown

      const deepEdgingFeet = Number(form.deepEdgingFeet || 0)
      if (deepEdgingFeet > 0) oneTime += Math.max(deepEdgingFeet * deepEdgingRate, deepEdgingMin)

      const shrubBlocksCount = Number(form.shrubBlocks || 0)
      if (shrubBlocksCount > 0) oneTime += Math.max(shrubBlocksCount * shrubBlock, shrubMin)
    ...
    const firstBill = form.quoteType === 'mowing' ? monthly + oneTime + travel : totalNow
    const recurringMonthly = form.quoteType === 'mowing' ? monthly + travel : 0

    return { perVisit, monthly, oneTime, travel, totalNow, firstBill, recurringMonthly }
  }, [form])

  async function sendEstimate() {
    if (!form.contactName || !form.email || !form.address) {
      setSendStatus({ state: 'error', message: 'Please enter your name, email, and property address first.' })
      return
    }

    setSendStatus({ state: 'sending', message: 'Sending estimate...' })
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error || 'Could not send estimate.')
      setSendStatus({ state: 'success', message: 'Estimate sent to the customer and to you.' })
    } catch (error) {
      setSendStatus({ state: 'error', message: error.message || 'Could not send estimate.' })
    }
  }
      ...
      <main className="main-grid">
        <section className="panel hero-panel">
          ...
          <div className="feature-grid">
            ...
            <div className="feature-card"><strong>Send Estimate by Email</strong><span>Customer and office copy sent in one click after details are entered.</span></div>
          </div>
        </section>
        ...
        <aside className="panel estimate-panel">
          ...
          {form.quoteType === 'mowing' ? (
            <>
              ...
              <div className="estimate-row muted"><span>Estimated recurring monthly after first bill</span><strong>{currency(estimate.recurringMonthly)}</strong></div>
            </>
          ) : (
            <>
          ...
          <button className="button button-primary send-button" onClick={sendEstimate} type="button">
            Send Estimate to Customer and Me
          </button>
          {sendStatus.message ? <div className={`send-status ${sendStatus.state}`}>{sendStatus.message}</div> : null}
        ...
        <section className="panel" id="contact">
          ...
          <div className="contact-grid">
            <a className="contact-card" href={`tel:${PHONE_LINK}`}><strong>Call</strong><span>{PHONE_DISPLAY}</span></a>
            <a className="contact-card" href={`sms:${PHONE_LINK}`}><strong>Text</strong><span>{PHONE_DISPLAY}</span></a>
            <a className="contact-card" href={`mailto:${PUBLIC_EMAIL}`}><strong>Email</strong><span>{PUBLIC_EMAIL}</span></a>
          </div>
        </section>
      </main>
    </div>
